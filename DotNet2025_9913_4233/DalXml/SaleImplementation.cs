...
{
    private  string filePath = @"..\xml\sales.xml";
    private XElement LoadXml()
    {
        if (!File.Exists(filePath))
        {
            XElement root = new XElement("ArrayOfSale");
            root.Save(filePath);
        }
    ...
    public int Create(Sale item)
    {
        ...
        XElement saleElement = new XElement("Sale",
            new XElement("Id", Config.NextSaleId),
    ...
    {
        XElement root = LoadXml();
        // חיפוש האלמנט המתאים לפי מזהה
        XElement? saleElement = root.Elements("Sale")
            .FirstOrDefault(e => (int)e.Element("Id") == item.id);

        if (saleElement == null)
            throw new DalNotFoundId("Sale not found");
        // עדכון הערכים בתוך האלמנט הקיים
